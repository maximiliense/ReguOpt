...
<div class="en-blend">
	...
	<div class="sparsity-bar">
		...
		<div class="sp-track">
			<div
				class="sp-fill"
				style:width={`${((features.length - activeCount) / features.length) * 100}%`}
			></div>
		</div>
	...
	<p class="cap">
		L'Elastic Net combine Ridge et Lasso via le paramètre <KatexInline
			formula={String.raw`\alpha`}
		/>. Quand <KatexInline formula={String.raw`\alpha = 0`} />, on obtient un rétrécissement doux
		(Ridge). Quand <KatexInline formula={String.raw`\alpha = 1`} />, les coefficients sont seuillés
		exactement à zéro (Lasso). Un <KatexInline formula={String.raw`\alpha \in ]0, 1[`} /> donne le meilleur
		des deux mondes : sélection de variables ET groupage stable pour features corrélées.
	</p>
</div>
